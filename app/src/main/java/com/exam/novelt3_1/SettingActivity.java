...
public class SettingActivity extends Activity {
    ...
    String name, hp, new_hp, pw, birthday, usr_local, usr_place, usr_max_bp, usr_min_bp, usr_fatness;
    float BMI;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        height = Float.parseFloat(intent.getExtras().getString("HEIGHT"));
        weight = Float.parseFloat(intent.getExtras().getString("WEIGHT"));
        usr_max_bp = intent.getExtras().getString("MAX_BP");
        usr_min_bp = intent.getExtras().getString("MIN_BP");
        usr_fatness = intent.getExtras().getString("FATNESS");
        usr_local = intent.getExtras().getString("LOCAL");
        usr_place = intent.getExtras().getString("PLACE");
        new_hp = hp;
        ...
        edit_phone.setText(hp);
        edit_birthday.setText(birthday);
        text_height.setText(String.valueOf(height));
        text_weight.setText(String.valueOf(weight));
        text_max_bp.setText(usr_max_bp);
        text_min_bp.setText(usr_min_bp);
        text_fatness.setText(usr_fatness);
                        ...
                        if(hp == null) {
                            ...
                            if(hp.length() != 11) {
                                ...
                                if(!isCorrectPW) {
                                    ...
                                    } else if(text_height.getText().toString().equals("0") || text_height.getText().toString().equals("")) {
                                        Toast.makeText(getApplicationContext(), "키를 설정해 주세요.", Toast.LENGTH_SHORT).show();
                                    } else {
                                        if(text_weight.getText().toString().equals("0") || text_weight.getText().toString().equals("")) {
    ...
    public void onSendData() {
        ...
        values.put("BMI", BMI);
        if(!text_max_bp.getText().toString().equals("0") || !text_max_bp.getText().toString().equals(""))
            values.put("MAX_BP", text_max_bp.getText().toString());
        else values.put("MAX_BP", 0);
        if(!text_min_bp.getText().toString().equals("0") || !text_min_bp.getText().toString().equals(""))
            values.put("MIN_BP", text_min_bp.getText().toString());
        else values.put("MIN_BP", 0);
        if(!text_fatness.getText().toString().equals("0") || !text_fatness.getText().toString().equals(""))
            values.put("FAT_PERCENT", text_fatness.getText().toString());
        else values.put("FAT_PERCENT", 0);
                ...
                if(checkHP_result != null) {
                    if(!hp.equals(new_hp)) {
                        if(checkHP_result.equals("N")) {
                            ...
                            if(spinner_selectedItem_position_local != 0 && spinner_selectedItem_position_place != 0) {
                                onSendData();
    ...
    class HttpPostDataClass_onLoadAreas extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            for(int i = 0; i < user_local.size(); i++) {
                if(user_local.get(i).equals(usr_local)) {
                    area.setSelection(i);
                    spinner_selectedItem_position_local = i;
                }
            }
        }
    }
    ...
    class HttpPostDataClass_onLoadPlaces extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String s) {
            super.onPostExecute(s);
            for(int i = 0; i < user_place.size(); i++) {
                if(user_place.get(i).equals(usr_place)) {
                    place.setSelection(i);
                    spinner_selectedItem_position_place = i;
                }
            }
        }
    }
